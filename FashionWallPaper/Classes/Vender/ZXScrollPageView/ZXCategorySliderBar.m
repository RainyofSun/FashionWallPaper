...
- (void)setSelectIndex:(NSInteger)index{
    ...
    [self adjustContentOffsetToIndex:index completeHanle:^{
        ...
        itemView.contentLabel.textColor = HexColor(0x2DD576);
...
- (void)setItemInterval:(CGFloat)interval {
    if (interval != _itemInteval) {
        _itemInteval = interval;
    }
}
...
- (UIColor *)changeRGB:(UIColor *)color changeValue:(CGFloat)value{
    ...
    color = [UIColor colorWithRed:[nextViewRgbArray[0] doubleValue] green:[nextViewRgbArray[1] doubleValue] + value blue:[nextViewRgbArray[2] doubleValue] alpha:[nextViewRgbArray[3] doubleValue]];
    return color;
}
...
- (UIView *)indicateView
{
    if (!_indicateView) {
        ...
        _indicateView.backgroundColor = HexColor(0x2DD576);
    }
...
- (void)initItemWidthArray{
    CGFloat originX = self.itemInteval;
    for (int i = 0; i< self.itemArray.count; i++) {
        ...
        originX = originX + textSize.width + self.itemInteval;
    }
...
- (UICollectionViewFlowLayout *)flowLayout
{
    if (!_flowLayout) {
        ...
        _flowLayout.minimumLineSpacing = self.itemInteval;
        _flowLayout.minimumInteritemSpacing = self.itemInteval;
        _flowLayout.sectionInset = UIEdgeInsetsMake(0, self.itemInteval, 0, self.itemInteval);
    }
    return _flowLayout;
}
                  ...
                  cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == self.currentIndex) {
        cell.contentLabel.textColor = HexColor(0x2DD576);
    }
    else{
...
- (void)adjustContentOffsetToIndex:(NSInteger)index completeHanle:(void(^)(void))completeHandle animation:(BOOL)animation{
    CGFloat maxContentOffsetX = [[self.itemOriginXArray lastObject]floatValue] + [[self.itemWidthArray lastObject] floatValue] + self.itemInteval - self.frame.size.width;
    if (index >= self.itemArray.count||index < 0) {
        return;
    }
